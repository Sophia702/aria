...
import '../../services/watch/watch_cadence_service.dart';

class WatchCadenceScreen extends ConsumerStatefulWidget {
  const WatchCadenceScreen({super.key});

  @override
  ConsumerState<WatchCadenceScreen> createState() => _WatchCadenceScreenState();
}

class _WatchCadenceScreenState extends ConsumerState<WatchCadenceScreen>
  ...
  StreamSubscription<double?>? _spmSub;
  ...
  void dispose() {
    ...
    _spmSub?.cancel();
    WatchCadenceService.stopMonitor();
    _beat.dispose();
    _pulseCtrl.dispose();
    super.dispose();
  }
  ...
  void _startTracking() {
    ...
    _spmSub = WatchCadenceService.liveStream().listen((spm) {
      if (!mounted) return;
      if (spm != null && spm > 0) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: false,
        title: Text('Watch Cadence', style: AppType.h2),
      ),
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Center(child: _SourceChip(active: _tracking)),
...
class _WatchCadenceOrb extends StatelessWidget {
  const _WatchCadenceOrb({
    required this.spm,
...
class _SourceChip extends StatelessWidget {
  const _SourceChip({required this.active});
  final bool active;
    ...
    return AnimatedOpacity(
      ...
      child: Container(
        ...
        child: const Row(
          ...
          children: [
            Icon(Icons.watch_rounded, color: AppColors.primary, size: 15),
            SizedBox(width: 6),
            Text(
              'Apple Watch',
              style: TextStyle(
...
class _WatchTrackButton extends StatelessWidget {
  const _WatchTrackButton({
    required this.tracking,
    required this.stabilizing,
    required this.onTap,
  });
...
class _WatchBeatCard extends StatelessWidget {
  const _WatchBeatCard({
            ...
            children: [
              ...
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Beat sync', style: AppType.h2.copyWith(fontSize: 16)),
...
class _WatchSoundChip extends StatelessWidget {
  const _WatchSoundChip({
    required this.label,
    required this.icon,
...
class _IosOnlyCard extends StatelessWidget {
  const _IosOnlyCard();
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        children: [
          const Icon(Icons.watch_rounded, color: AppColors.inkFaint, size: 40),
          const SizedBox(height: AppSpacing.sm),
          Text('Apple Watch required',
              style: AppType.h2.copyWith(fontSize: 16)),
          const SizedBox(height: 4),
          Text(
            'Watch cadence tracking uses HealthKit\nand is only available on iOS.',
            style: AppType.body,
